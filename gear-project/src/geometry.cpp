...
int gemetry(const Matrix<uint> &l, int n, vector<shared_ptr<IObject>> &results)
{
	...
	//каждый объект ограничим квадратиком
	vector <border> b(n);
...
int aroundInt(float n)//типо окургление
{
	int a = n;
...
{  
	...
	//оптимизация подсчетов кратного интеграла))))
    ...
    {
		int rL = getRadiusMin(b, l, xc - 1, yc),
			rR = getRadiusMin(b, l, xc + 1, yc),
			rU = getRadiusMin(b, l, xc, yc - 1),
			rD = getRadiusMin(b, l, xc, yc + 1);
      
		int rLU = getRadiusMin(b, l, xc - 1, yc - 1),
			rRD = getRadiusMin(b, l, xc + 1, yc + 1),
			rRU = getRadiusMin(b, l, xc + 1, yc - 1),
			rLD = getRadiusMin(b, l, xc - 1, yc + 1);
...
//
float getAngle(float n)//дай угол
{
	return (n / 180) * 3.1415926535897;
}

int getRadiusMin(const border b, const Matrix<uint> &l, const int xc, const int yc)//задачи минимум\максимум
{
...
int getRadiusMax(const border b, const Matrix<uint> &l, const int xc,const int yc)//да так же
{
	uint num = l(yc,xc);
	...
	for(int a = 1; a < 360; ++a)
	{
		...
		while(l(y,x) == num)
		{
			...
			if(y1 <= b.down && x1 <= b.right&&y1 >= b.up&&x1 >= b.left) 
			{
				if(l(y1,x1) == 0) 
				break;
			}
																			...
																			//вернет отрицательное, если она сломанная
{
	...
	int cogs = 0;
	bool st1 = false;//картинка
	bool flag = false;//флаг переключения
	bool broken = false;
	
	int fi = 0;//угол между зубьями
	int sfi = 0;//угол разности
	...
	for(int a = 0; a < 360; ++a)
	{
		...
		if(l(y,x) == num && !flag)//если наткнулись на зуб, ИНОГДА ПАДАЕТ ЗУБ НА ЗУБ
		{
			++cogs;
			flag = true;
