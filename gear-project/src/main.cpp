...
int csfr(const vector<shared_ptr<IObject>> &obj, Image& in, int res, int n,const string name);//подмена точки
...
{
	int x0, y0; 
	tie(x0,y0) = obj[res] -> location;//
	int R = 1000000, r = 1000000;//+-INF задача мин/макс)
...
//происходит вставка картины(сдвиг)
		for(int i = 0; i < n_r; ++i) 
			for(int j = 0; j < n_c; ++j) 
				if(b(i,j)) in(i + y0 - yc, j + x0 - xc) = im(i,j);
...
uint get_limit(const Image& in)
{
	...
	for(i = 0; i < in.n_rows; i++)
	{
		...
		{
			tie(r, g, b) = in(i, j);
			int bright = 0.3 * r + 0.59 * g + 0.11 * b;
...
void bnr(const Image &in, Matrix<uint>&bn)
{
	int n_r = in.n_rows, n_c = in.n_cols;
	uint r;
	uint g;
